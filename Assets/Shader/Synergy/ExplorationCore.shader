Shader "GeoWorld/Synergy/Exploration"
{
    Properties
    {
        ...
        _CoreRadius            ("Inner Color Pivot", Range(0.05, 0.3)) = 0.12
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                float4 _ColorA, _ColorB, _VoidColor;
            ...
            half4 Frag(Varyings IN, out float outDepth : SV_Depth) : SV_Target
            {
                ...
                // Containment + depth use the TRUE unit cube (the rasterised mesh),
                // NOT _VolumeScale. With _VolumeScale > 1 the old box pushed the ray
                // ENTRY point (and thus the written depth) in FRONT of the real cube,
                // so the volume occluded blocks that were actually nearer. _VolumeScale
                // now only scales the internal crystal pattern, never the geometry.
                float3 boxMin = float3(-0.5, -0.5, -0.5);
                float3 boxMax = float3( 0.5,  0.5,  0.5);

                float2 hitAABB = IntersectAABB(rayOriginOS, rayDirOS, boxMin, boxMax);

                // DEPTH FIX: this pass uses `Cull Front`, so it rasterises the cube's
                // BACK faces — the hardware depth would land on the FAR side of the
                // cube and let opaque blocks sitting physically BEHIND the volume win
                // the depth test and draw over it. Re-emit depth at the ray ENTRY
                // point (the cube's NEAR face) so the volume occludes and sorts
                // exactly like a solid cube.
                outDepth = IN.positionCS.z / IN.positionCS.w;   // silhouette-edge fallback
                ...
                float3 entryWS = TransformObjectToWorld(rayOriginOS + rayDirOS * tStart);
                float4 entryCS = TransformWorldToHClip(entryWS);
                outDepth = entryCS.z / entryCS.w;
                ...
                for(int i = 0; i < _MaxSteps; i++)
                {
                    ...
                    if (r < _OuterFade)
                    {
                        // Core removed — the crystalline shells fill all the way in.
