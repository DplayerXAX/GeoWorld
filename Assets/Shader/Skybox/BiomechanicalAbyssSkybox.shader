Shader "Custom/BiomechanicalAbyssSkybox_Menu"
{
    Properties
    {
        ...
        _PulseSpeed    ("Throb Speed", Float) = 0.5 // 稍微放慢默认速度，使其更具呼吸感
        
        // --- Main Menu Optimizations ---
        [Toggle] _MenuMode ("Enable Main Menu Mode", Float) = 1.0
        _MenuBgDamp        ("Menu UI Contrast Damp", Range(0,1)) = 0.6 // 压暗背景、淡化高频细节，突显UI
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                // 4. 高频微小细胞噪声（主界面模式下予以大幅减弱）
                float cellNoise = Noise(dir * 25.0 - float3(0, time * 0.2, 0));
                float cellMask = (_MenuMode > 0.5) ? 0.15 : 1.0; // 降低细胞亮度和颗粒感
                ...
                // 5. 【主界面专属优化】增加上下空间压暗渐变，为UI留白，并整体控光
                if (_MenuMode > 0.5)
                {
                    // 利用天空盒的Y轴方向（dir.y 从 -1 到 1）做一个正视前方的暗色过渡
                    // 假设UI主要在屏幕中央和两侧，通过对 dir.y 的绝对值或线性进行压暗
                    float verticalDim = smoothstep(-0.6, 0.5, dir.y); 
                    
                    // 整体暗度控制，保证亮色生物质不会盖过白色/金色UI字体的清晰度
